...
static int Sys_PS(struct Interrupt_State *state) {
    ...
    struct Process_Info* processInfo = (struct Process_Info*)state->ebx;;
    int len = state->ecx;
	...
	return 0;
        ...
        while (kthread != 0) {     
            ...
            if(kthread->userContext != 0 ){
	            strcpy(tmp.name, kthread->userContext->name);
	        }
	        else{	
                strcpy(tmp.name, kthread->threadName);
            } 
	        tmp.pid = kthread->pid;
            if(kthread->owner == 0){
	            tmp.parent_pid = 0;
                ...
                tmp.parent_pid = kthread->owner->pid;
            } 
	        tmp.priority = kthread->priority;


            if (kthread->alive == false && ((kthread->refCount == 0)||(kthread->refCount == 1))) {
                tmp.status = STATUS_ZOMBIE;
            }
            else if (kthread->alive == true) {
                struct Kernel_Thread* runNode = Get_Front_Of_Thread_Queue(&s_runQueue);
                char flag = 'b';
                while (runNode != 0) {
                    if (runNode->pid == kthread->pid) {
                        flag = 'r';
                    }
                    runNode = Get_Next_In_Thread_Queue(runNode);
                }

                if (g_currentThreads[Get_CPU_ID()]->pid == kthread->pid) {
                    flag = 'r';
                }
                if (flag == 'b') { 
                    tmp.status = STATUS_BLOCKED;
                }
                else if (flag == 'r') {
                    tmp.status = STATUS_RUNNABLE;
                }
            }

            tmp.affinity = kthread->affinity;
            tmp.currCore = 0;
            tmp.totalTime = kthread->totalTime;
        

            Copy_To_User((ulong_t)processInfo, &tmp, sizeof(struct Process_Info));
            rc++;
...
static int Sys_Kill(struct Interrupt_State *state) {
    ...
    struct Kernel_Thread* kthread;
    struct Kernel_Thread* current = g_currentThreads[Get_CPU_ID()];
    char status = ' ';
    Enable_Interrupts();
    ...
    while (runNode != 0) {
        if (runNode->pid == kthread->pid) {
            status = 'r';
            Remove_Thread(&s_runQueue, kthread);
            struct Kernel_Thread* preNode = Get_Prev_In_All_Thread_List(runNode);
            struct Kernel_Thread* nextNode = Get_Next_In_All_Thread_List(runNode);
            Set_Next_In_All_Thread_List(preNode,nextNode);
	        Set_Prev_In_All_Thread_List(nextNode,preNode);
            
            int i, j, called = 0;
            if (runNode->refCount > 0) {
                runNode->refCount--;
            }
            ...
            for (j = 0; j < MIN_DESTRUCTOR_ITERATIONS; j++) {
                ...
                for (i = 0; i < MAX_TLOCAL_KEYS; i++) {
                    ...
                    void* x = (void*)runNode->tlocalData[i];
                    if (x != NULL ) {

                        runNode->tlocalData[i] = NULL;
                        called = 1;
    ...
    Alarm_Cancel_For_Thread(kthread);
	    
    /* Notify the thread's owner, if any */ 
    Wake_Up(&kthread->joinQueue);
