...
int Load_User_Program(char *exeFileData, ulong_t exeFileLength
    ...
    {
	//if(exeFormat->segmentList[i].startAddress == 0 && exeFormat->segmentList[i] ==0) continue;
        ulong_t topva = exeFormat->segmentList[i].startAddress + exeFormat->segmentList[i].sizeInMemory;
	if(topva > maxva){
		maxva = topva;
	}
    }
